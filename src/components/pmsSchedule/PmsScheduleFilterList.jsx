...
const PmsScheduleFilterList = ({
  ...
  const handleView = (s) => navigate(`/pms-schedule/${s.id}`);
  const handleEdit = (s) => navigate(`/pms-schedule/${s.id}`);
  const handleDeleteClick = (s) => {
    setToDelete(s);
    setDeleteOpen(true);
  };

  // Naya function — status dropdown se change hone par backend update karta hai
  const handleStatusChange = async (schedule, newStatus) => {
    try {
      // Poori schedule object bhejni hai (sirf status nahi), kyunke
      // updatePmsSchedule PUT poora record replace karta hai
      await updatePmsSchedule(schedule.id, { ...schedule, status: newStatus });
  ...
  const columns = getPmsScheduleColumns({
    ...
    onEdit: handleEdit,
    onDelete: handleDeleteClick,
    onStatusChange: handleStatusChange, // 👈 add kiya
  });
